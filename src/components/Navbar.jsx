...
function Navbar() {
  ...
  return (
    <div>
      <nav className='flex justify-between p-4 items-center border-b bg-white dark:bg-gradient-to-r from-cyan-500 to-blue-500'>
        ...
        <ul className='hidden md:flex gap-6'>
          ...
          {isAuthenticated ? (
            <Link to='/profile'><li>{user?.username || "Perfil"}</li></Link>
          ) : (
            <>
        ...
        >
          ...
          {isAuthenticated ? (
            <Link to='/profile'><li className='hover:text-teal-700'>{user?.username || "Perfil"}</li></Link>
          ) : (
            <>
